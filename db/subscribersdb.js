...
const updateSubscriber = async (subscriberKey, kvToUpdate) => {
  ...
  const db = arango_connection.getDb();
  const collection = db.collection("subscribers");
  const subscriber = await collection.update(subscriberKey, kvToUpdate, {
    returnNew: true,
  });
  return subscriber;
};
...
const getAllEmailConfirmsKeys = async () => {
  const db = arango_connection.getDb();
  const collection = db.collection("email_confirm");
  const q = aql`
        FOR v IN ${collection} RETURN v._key
    `;
  const cursor = await db.query(q);
  const all_confirms = await cursor.all();
  return all_confirms;
};
...
const setValidateEmailEntryDone = async (entryKey) => {
  ...
  const done = await collection.update(
    entryKey,
    { done: true, date_updated },
    { returnNew: true }
  );
};
...
module.exports = {
  ...
  emailExistsAndVerified,
  createValidateEmailEntry,
  getValidateEmailEntry,
  setValidateEmailEntryDone,
  getAllEmailConfirmsKeys,
};
