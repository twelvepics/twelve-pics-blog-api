...
// LOGGING
var { api_logger, subscribers_events_logger, event_info } = require('../logger');
...
const sendValidateEmail = async (subscriber) => {
  ...
  const validateEmailEntry = await subscribersdb.createValidateEmailEntry(subscriber._key);
  const front_url = process.env.FRONT_URL;
  const validate_url = `${front_url}/email-confirm/${validateEmailEntry._key}`;
...
router.post('/', async (req, res) => {
  try {
    ...
    subscribers_events_logger.info(`create subscriber\n${event_info(req, subscriber)}`);
...
router.post('/validate-email/:validate_key', async (req, res) => {
  try {
    ...
    if (!entry) {
      ...
      const subscriber_updated = await subscribersdb.updateSubscriber(entry.subscriber_key, {
